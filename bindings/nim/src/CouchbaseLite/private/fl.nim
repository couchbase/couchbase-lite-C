...
## ////// BASIC TYPES
type
  Value* = ptr object ## A reference to a value of any type.
  Array* = ptr object ## A reference to an array value.
  Dict* = ptr object ## A reference to a dictionary (map) value.
  Slot* = ptr object ## A reference to a mutable array/dict item
  MutableArray* = ptr object ## A reference to a mutable array.
  MutableDict* = ptr object ## A reference to a mutable dictionary.
...
##
type
  Doc* = ptr object ## A reference to a document.
...
proc newDocFromResultData*(data: FLSliceResult; a2: Trust; a3: SharedKeys; externData: FLSlice): Doc {.importc: "FLDoc_FromResultData".}
...
##         function returns.
proc newDocFromJSON*(json: FLSlice; outError: var FLError): Doc {.importc: "FLDoc_FromJSON".}

## Releases a reference to an FLDoc. This must be called once to free an FLDoc you created.
proc release*(a1: Doc) {.importc: "FLDoc_Release".}
...
proc retain*(a1: Doc): Doc {.importc: "FLDoc_Retain".}

## Returns the encoded Fleece data backing the document.
proc getData*(a1: Doc): FLSlice {.importc: "FLDoc_GetData".}

## Returns the FLSliceResult data owned by the document, if any, else a null slice.
proc getAllocedData*(a1: Doc): FLSliceResult {.importc: "FLDoc_GetAllocedData".}

## Returns the root value in the FLDoc, usually an FLDict.
proc getRoot*(a1: Doc): Value {.importc: "FLDoc_GetRoot".}

## Returns the FLSharedKeys used by this FLDoc, as specified when it was created.
proc getSharedKeys*(a1: Doc): SharedKeys {.importc: "FLDoc_GetSharedKeys".}
...
proc findDoc*(a1: Value): Doc {.importc: "FLValue_FindDoc".}
...
proc newValueFromData*(data: FLSlice; a2: Trust): Value {.importc: "FLValue_FromData".}
...
proc toJSON*(a1: Value): FLStringResult {.importc: "FLValue_ToJSON".}
...
proc toJSON5*(v: Value): FLStringResult {.importc: "FLValue_ToJSON5".}

## Most general Fleece to JSON converter.
proc toJSONX*(v: Value; json5: bool; canonicalForm: bool): FLStringResult {.importc: "FLValue_ToJSONX".}
...
proc dump*(a1: Value): cstring {.importc: "FLDump".}
...
proc getType*(a1: Value): ValueType {.importc: "FLValue_GetType".}

## Returns true if the value is non-NULL and represents an integer.
proc isInteger*(a1: Value): bool {.importc: "FLValue_IsInteger".}
...
proc asInt*(a1: Value): int64 {.importc: "FLValue_AsInt".}
...
proc asUnsigned*(a1: Value): uint64 {.importc: "FLValue_AsUnsigned".}
...
proc asFloat*(a1: Value): cfloat {.importc: "FLValue_AsFloat".}

## Returns a value coerced to a 32-bit floating point number.
##         True and false are returned as 1.0 and 0.0, and integers are converted to float. All other
##         types are returned as 0.0.
##         the limitations of IEEE 32-bit float format.
proc asDouble*(a1: Value): cdouble {.importc: "FLValue_AsDouble".}

## Returns the exact contents of a string value, or null for all other types.
proc asString*(a1: Value): FLString {.importc: "FLValue_AsString".}
...
proc asTimestamp*(a1: Value): Timestamp {.importc: "FLValue_AsTimestamp".}

## Returns the exact contents of a data value, or null for all other types.
proc asData*(a1: Value): FLSlice {.importc: "FLValue_AsData".}

## If a FLValue represents an array, returns it cast to FLArray, else NULL.
proc asArray*(a1: Value): Array {.importc: "FLValue_AsArray".}

## If a FLValue represents a dictionary, returns it as an FLDict, else NULL.
proc asDict*(a1: Value): Dict {.importc: "FLValue_AsDict".}
...
proc release*(a1: Value) {.importc: "FLValue_Release".}
...
var kNullValue* {.importc: "kFLNullValue".}: Value



## ////// VALUE SLOT
proc setNull*(a1: Slot) {.importc: "FLSlot_SetNull".} ## Stores a JSON null into a slot.

proc set*(a1: Slot; a2: bool) {.importc: "FLSlot_SetBool".} ## Stores a boolean into a slot.

proc set*(a1: Slot; a2: int64) {.importc: "FLSlot_SetInt".} ## Stores an integer into a slot.

proc set*(a1: Slot; a2: uint64) {.importc: "FLSlot_SetUInt".} ## Stores an unsigned integer into a slot.

proc set*(a1: Slot; a2: cfloat) {.importc: "FLSlot_SetFloat".} ## Stores a float into a slot.

proc set*(a1: Slot; a2: cdouble) {.importc: "FLSlot_SetDouble".} ## Stores a double into a slot.

proc set*(a1: Slot; a2: FLString) {.importc: "FLSlot_SetString".} ## Stores a string into a slot.

proc setData*(a1: Slot; a2: FLSlice) {.importc: "FLSlot_SetData".} ## Stores a data blob into a slot.

proc set*(a1: Slot; a2: Value) {.importc: "FLSlot_SetValue".} ## Stores an FLValue into a slot.
...
proc count*(a1: Array): uint32 {.importc: "FLArray_Count".}
...
proc isEmpty*(a1: Array): bool {.importc: "FLArray_IsEmpty".}

## If the array is mutable, returns it cast to FLMutableArray, else NULL.
proc asMutable*(a1: Array): MutableArray {.importc: "FLArray_AsMutable".}

## Returns an value at an array index, or NULL if the index is out of range.
proc get*(a1: Array; index: uint32): Value {.importc: "FLArray_Get".}
var kEmptyArray* {.importc: "kFLEmptyArray".}: Array
...
proc begin*(a1: Array; a2: ptr ArrayIterator) {.importc: "FLArrayIterator_Begin".}

## Returns the current value being iterated over.
proc getValue*(a1: ptr ArrayIterator): Value {.importc: "FLArrayIterator_GetValue".}

## Returns a value in the array at the given offset from the current value.
proc getValueAt*(a1: ptr ArrayIterator; offset: uint32): Value {.importc: "FLArrayIterator_GetValueAt".}

## Returns the number of items remaining to be iterated, including the current one.
proc getCount*(a1: ptr ArrayIterator): uint32 {.importc: "FLArrayIterator_GetCount".}

## Advances the iterator to the next value, or returns false if at the end.
proc next*(a1: ptr ArrayIterator): bool {.importc: "FLArrayIterator_Next".}
...
proc mutableCopy*(a1: Array; a2: CopyFlags): MutableArray {.importc: "FLArray_MutableCopy".}

## Creates a new empty mutable Array.
##         Its initial ref-count is 1, so a call to FLMutableArray_Free will free it.
proc newMutableArray*(): MutableArray {.importc: "FLMutableArray_New".}
...
proc getSource*(a1: MutableArray): Array {.importc: "FLMutableArray_GetSource".}

## Returns true if the Array has been changed from the source it was copied from.
proc isChanged*(a1: MutableArray): bool {.importc: "FLMutableArray_IsChanged".}
...
proc set*(a1: MutableArray; index: uint32): Slot {.importc: "FLMutableArray_Set".}
...
proc append*(a1: MutableArray): Slot {.importc: "FLMutableArray_Append".}

## Inserts a contiguous range of JSON `null` values into the array.
proc insert*(array: MutableArray; firstIndex: uint32; count: uint32) {.importc: "FLMutableArray_Insert".}

## Removes contiguous items from the array.
proc remove*(array: MutableArray; firstIndex: uint32; count: uint32) {.importc: "FLMutableArray_Remove".}
...
proc resize*(array: MutableArray; size: uint32) {.importc: "FLMutableArray_Resize".}
...
proc getMutableArray*(a1: MutableArray; index: uint32): MutableArray {.importc: "FLMutableArray_GetMutableArray".}
...
##         - If the value is an immutable array, this function makes a mutable copy, assigns the
##           copy as the property value, and returns the copy.
proc getMutableDict*(a1: MutableArray; index: uint32): MutableDict {.importc: "FLMutableArray_GetMutableDict".}
...
proc count*(a1: Dict): uint32 {.importc: "FLDict_Count".}
...
proc isEmpty*(a1: Dict): bool {.importc: "FLDict_IsEmpty".}

## If the dictionary is mutable, returns it cast to FLMutableDict, else NULL.
proc asMutable*(a1: Dict): MutableDict {.importc: "FLDict_AsMutable".}

## Looks up a key in a dictionary, returning its value.
##         Returns NULL if the value is not found or if the dictionary is NULL.
proc get*(a1: Dict; keyString: FLSlice): Value {.importc: "FLDict_Get".}
var kEmptyDict* {.importc: "kFLEmptyDict".}: Dict
...
proc begin*(a1: Dict; a2: ptr DictIterator) {.importc: "FLDictIterator_Begin".}

## Returns the current key being iterated over. This Value will be a string or an integer.
proc getKey*(a1: ptr DictIterator): Value {.importc: "FLDictIterator_GetKey".}

## Returns the current key's string value.
proc getKeyString*(a1: ptr DictIterator): FLString {.importc: "FLDictIterator_GetKeyString".}

## Returns the current value being iterated over.
proc getValue*(a1: ptr DictIterator): Value {.importc: "FLDictIterator_GetValue".}

## Returns the number of items remaining to be iterated, including the current one.
proc getCount*(a1: ptr DictIterator): uint32 {.importc: "FLDictIterator_GetCount".}

## Advances the iterator to the next value, or returns false if at the end.
proc next*(a1: ptr DictIterator): bool {.importc: "FLDictIterator_Next".}
...
proc `end`*(a1: ptr DictIterator) {.importc: "FLDictIterator_End".}
...
proc init*(string: FLSlice): DictKey {.importc: "FLDictKey_Init".}

## Returns the string value of the key (which it was initialized with.)
proc getString*(a1: ptr DictKey): FLString {.importc: "FLDictKey_GetString".}
...
proc getWithKey*(a1: Dict; a2: ptr DictKey): Value {.importc: "FLDict_GetWithKey".}
...
proc mutableCopy*(source: Dict; a2: CopyFlags): MutableDict {.importc: "FLDict_MutableCopy".}

## Creates a new empty mutable Dict.
##         Its initial ref-count is 1, so a call to FLMutableDict_Free will free it.
proc newMutableDict*(): MutableDict {.importc: "FLMutableDict_New".}
...
proc getSource*(a1: MutableDict): Dict {.importc: "FLMutableDict_GetSource".}

## Returns true if the Dict has been changed from the source it was copied from.
proc isChanged*(a1: MutableDict): bool {.importc: "FLMutableDict_IsChanged".}
...
proc set*(nonnull: MutableDict; key: FLString): Slot {.importc: "FLMutableDict_Set".}

## Removes the value for a key.
proc remove*(a1: MutableDict; key: FLString) {.importc: "FLMutableDict_Remove".}

## Removes all keys and values.
proc removeAll*(a1: MutableDict) {.importc: "FLMutableDict_RemoveAll".}
...
##         - If the value is an immutable array, this function makes a mutable copy, assigns the
##           copy as the property value, and returns the copy.
proc getMutableArray*(a1: MutableDict; key: FLString): MutableArray {.importc: "FLMutableDict_GetMutableArray".}
...
##           copy as the property value, and returns the copy.
proc getMutableDict*(a1: MutableDict; key: FLString): MutableDict {.importc: "FLMutableDict_GetMutableDict".}
...
proc newDeepIterator*(a1: Value): DeepIterator {.importc: "FLDeepIterator_New".}
...
proc getValue*(a1: DeepIterator): Value {.importc: "FLDeepIterator_GetValue".}
...
##
type
  KeyPath* = ptr object ## A reference to a key path.

## Creates a new FLKeyPath object by compiling a path specifier string.
proc newKeyPath*(specifier: FLSlice; error: var FLError): KeyPath {.importc: "FLKeyPath_New".}

## Frees a compiled FLKeyPath object. (It's ok to pass NULL.)
proc free*(a1: KeyPath) {.importc: "FLKeyPath_Free".}

## Evaluates a compiled key-path for a given Fleece root object.
proc eval*(a1: KeyPath; root: Value): Value {.importc: "FLKeyPath_Eval".}
...
proc evalOnce*(specifier: FLSlice; root: Value; error: var FLError): Value {.importc: "FLKeyPath_EvalOnce".}
...
##
type
  Encoder* = ptr object  ## A reference to an encoder.
...
proc newEncoder*(): Encoder {.importc: "FLEncoder_New".}
...
proc newEncoderWithOptions*(format: EncoderFormat; reserveSize: csize_t; uniqueStrings: bool): Encoder {.importc: "FLEncoder_NewWithOptions".}

## Creates a new Fleece encoder that writes to a file, not to memory.
proc newEncoderWritingToFile*(a1: ptr File; uniqueStrings: bool): Encoder {.importc: "FLEncoder_NewWritingToFile".}

## Frees the space used by an encoder.
proc free*(a1: Encoder) {.importc: "FLEncoder_Free".}

## Tells the encoder to use a shared-keys mapping when encoding dictionary keys.
proc setSharedKeys*(a1: Encoder; a2: SharedKeys) {.importc: "FLEncoder_SetSharedKeys".}

## Associates an arbitrary user-defined value with the encoder.
proc setExtraInfo*(a1: Encoder; info: pointer) {.importc: "FLEncoder_SetExtraInfo".}

## Returns the user-defined value associated with the encoder; NULL by default.
proc getExtraInfo*(a1: Encoder): pointer {.importc: "FLEncoder_GetExtraInfo".}
...
proc amend*(e: Encoder; base: FLSlice; reuseStrings: bool; externPointers: bool) {.importc: "FLEncoder_Amend".}

## Returns the `base` value passed to FLEncoder_Amend.
proc getBase*(a1: Encoder): FLSlice {.importc: "FLEncoder_GetBase".}
...
proc suppressTrailer*(a1: Encoder) {.importc: "FLEncoder_SuppressTrailer".}

## Resets the state of an encoder without freeing it. It can then be reused to encode
##         another value.
proc reset*(a1: Encoder) {.importc: "FLEncoder_Reset".}

## Returns the number of bytes encoded so far.
proc bytesWritten*(a1: Encoder): csize_t {.importc: "FLEncoder_BytesWritten".}
...
proc getNextWritePos*(a1: Encoder): csize_t {.importc: "FLEncoder_GetNextWritePos".}
...
proc writeNull*(a1: Encoder): bool {.importc: "FLEncoder_WriteNull".}
...
proc writeUndefined*(a1: Encoder): bool {.importc: "FLEncoder_WriteUndefined".}

## Writes a boolean value (true or false) to an encoder.
proc writeBool*(a1: Encoder; a2: bool): bool {.importc: "FLEncoder_WriteBool".}
...
proc writeInt*(a1: Encoder; a2: int64): bool {.importc: "FLEncoder_WriteInt".}
...
proc writeUInt*(a1: Encoder; a2: uint64): bool {.importc: "FLEncoder_WriteUInt".}
...
##         as floating-point.
proc writeFloat*(a1: Encoder; a2: cfloat): bool {.importc: "FLEncoder_WriteFloat".}
...
proc writeDouble*(a1: Encoder; a2: cdouble): bool {.importc: "FLEncoder_WriteDouble".}

## Writes a string to an encoder. The string must be UTF-8-encoded and must not contain any
##         zero bytes.
proc writeString*(a1: Encoder; a2: FLString): bool {.importc: "FLEncoder_WriteString".}

## Writes a timestamp to an encoder, as an ISO-8601 date string.
##         metadata that distinguishes it as a date. It's just a string.)
proc writeDateString*(encoder: Encoder; ts: Timestamp; asUTC: bool): bool {.importc: "FLEncoder_WriteDateString".}
...
proc writeData*(a1: Encoder; a2: FLSlice): bool {.importc: "FLEncoder_WriteData".}
...
proc writeRaw*(a1: Encoder; a2: FLSlice): bool {.importc: "FLEncoder_WriteRaw".}
...
##             just use zero.
proc beginArray*(a1: Encoder; reserveCount: csize_t): bool {.importc: "FLEncoder_BeginArray".}

## Ends writing an array value; pops back the previous encoding state.
proc endArray*(a1: Encoder): bool {.importc: "FLEncoder_EndArray".}
...
##             just use zero.
proc beginDict*(a1: Encoder; reserveCount: csize_t): bool {.importc: "FLEncoder_BeginDict".}

## Specifies the key for the next value to be written to the current dictionary.
proc writeKey*(a1: Encoder; a2: FLString): bool {.importc: "FLEncoder_WriteKey".}

## Specifies the key for the next value to be written to the current dictionary.
##         The key is given as a Value, which must be a string or integer.
proc writeKeyValue*(a1: Encoder; a2: Value): bool {.importc: "FLEncoder_WriteKeyValue".}

## Ends writing a dictionary value; pops back the previous encoding state.
proc endDict*(a1: Encoder): bool {.importc: "FLEncoder_EndDict".}

## Writes a Fleece Value to an Encoder.
proc writeValue*(a1: Encoder; a2: Value): bool {.importc: "FLEncoder_WriteValue".}
...
##         array.)
proc convertJSON*(a1: Encoder; json: FLSlice): bool {.importc: "FLEncoder_ConvertJSON".}

## Finishes encoding the current item, and returns its offset in the output data.
proc finishItem*(a1: Encoder): csize_t {.importc: "FLEncoder_FinishItem".}
...
proc finishDoc*(a1: Encoder; a2: var FLError): Doc {.importc: "FLEncoder_FinishDoc".}

## Ends encoding; if there has been no error, it returns the encoded data, else null.
##         This does not free the FLEncoder; call FLEncoder_Free (or FLEncoder_Reset) next.
proc finish*(e: Encoder; outError: var FLError): FLSliceResult {.importc: "FLEncoder_Finish".}

## Returns the error code of an encoder, or NoError (0) if there's no error.
proc getError*(a1: Encoder): FLError {.importc: "FLEncoder_GetError".}

## Returns the error message of an encoder, or NULL if there's no error.
proc getErrorMessage*(a1: Encoder): cstring {.importc: "FLEncoder_GetErrorMessage".}
...
proc createJSONDelta*(old: Value; nuu: Value): FLSliceResult {.importc: "FLCreateJSONDelta".}

## Writes JSON that describes the changes to turn the value `old` into `nuu`.
##         (The format is documented in Fleece.md, but you should treat it as a black box.)
##                 `FLEncoder_NewWithOptions`, with JSON or JSON5 format.
proc encodeJSONDelta*(old: Value; nuu: Value; jsonEncoder: Encoder): bool {.importc: "FLEncodeJSONDelta".}
...
proc applyJSONDelta*(old: Value; jsonDelta: FLSlice; error: var FLError): FLSliceResult {.importc: "FLApplyJSONDelta".}
...
##                     equal to the `old` value used when creating the `jsonDelta`.
##                     supported.)
proc encodeApplyingJSONDelta*(old: Value; jsonDelta: FLSlice; encoder: Encoder): bool {.importc: "FLEncodeApplyingJSONDelta".}
