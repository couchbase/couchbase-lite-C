...
proc parseJSON*(json: string): Fleece =
    ## Parses JSON data, producing a Fleece object.
    ## (JSON is always assumed untrusted and will be validated. Errors cause exceptions.)
    var err: FLError
...
proc toJSON*(v: FleeceObject, flags: ToJSONFlags ={}): string =
    ## Converts any value to a JSON string. Returns "undefined" if the value is undefined.
...
proc keyPath*(path: string): KeyPath =
    ## Creates a new KeyPath from a path string like "coords.lat" or "contacts[2].lastName".
    ## Throws an exception if the path syntax is invalid.
    var err: FLError
...
proc newMutableArray*(): MutableArray =
    ## Creates an empty mutable array. Unlike the immutable values that point into Fleece-formatted
    ## data, this object is not dependent on a container and stays valid as long as you have a
    ## reference to it.
...
proc delete*(a: MutableArray; index: uint32)                    =
    ## Removes a value from a mutable array. Any items at higher indexes move down one.
...
proc newMutableDict*(): MutableDict =
    ## Creates an empty mutable dictionary. Unlike the immutable values that point into Fleece-formatted
    ## data, this object is not dependent on a container and stays valid as long as you have a
    ## reference to it.
