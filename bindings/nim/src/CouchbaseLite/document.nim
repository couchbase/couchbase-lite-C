...
type
    ...
    SaveConflictHandler* =
        proc (documentBeingSaved: MutableDocument; conflictingDocument: Document): bool
        ...
        ## since it was loaded into the Document being saved.
...
proc saveDocument*(db: Database; doc: MutableDocument; handler: SaveConflictHandler): Document {.discardable.} =
    ## Saves a (mutable) document to the database.
    ## If a conflicting revision has been saved since the document was loaded, the callback is
    ## called with (a) the document being saved, and (b) the conflicting version already in the
    ## database (which may be nil, if the conflict is that the document was deleted.) The
    ## callback should make the appropriate changes to the document and return ``true``, or
...
proc deleteDocument*(db: Database; doc: Document; concurrency: ConcurrencyControl = LastWriteWins) {.discardable.} =
    ## Deletes a document from the database. Deletions are replicated, so document metadata will
    ## be left behind that can later be pushed by the replicator.
...
proc revisionID*(doc: Document): string =
    ## The document's revision ID, which is a short opaque string that's guaranteed to be
    ## unique to every change made to the document.
    ## If the document doesn't exist yet, returns an empty string.
...
proc sequence*(doc: Document): uint64 =
    ...
    ## abstract 'clock' to tell relative local modification times.
    doc.handle.sequence
...
proc properties*(doc: Document): Dict =
    ...
    ## This object is immutable; the subclass ``MutableDocument`` returns a mutable Dict.
...
proc newDocument*(): MutableDocument =
    ...
    ## Its ID will be a randomly-generated UUID.
...
proc mutableCopy*(doc: Document): MutableDocument =
    ## Creates a new MutableDocument instance that refers to the same document as the original.
    ## If the original document is mutable and has unsaved changes, the new one will also start out
    ## with the same changes; but mutating one document thereafter will not affect the other.
...
proc properties*(doc: MutableDocument): MutableDict =
    ...
    ## You may modify this dictionary and then call ``Database.saveDocument`` to persist the changes.
    wrap(doc.handle.mutableProperties)

proc `[]=`*(doc: MutableDocument, key: string, value: Settable) =
    ## Sets the value of a property. Equivalent to ``doc.properties[key] = value``.
    doc.properties[key] = value

proc `propertiesAsJSON=`*(doc: MutableDocument; json: string) =
    ## Sets the document's properties from the JSON string, which must contain an object.
