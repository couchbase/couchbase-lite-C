...
type
    ...
    QuerySyntaxError* = ref object of CouchbaseLiteError
        ## Exception thrown by ``Query.newQuery``. It contains the byte offset of the approximate
...
proc newQuery*(db: Database; str: string; language: QueryLanguage = N1QL): Query =
    ...
    # Query instance around instead of compiling it each time. If you need to run related queries
...
proc columnCount*(query: Query): uint =
    ## The number of columns in each result ``Row``. Equal to the number of items specified in the
    ## query string's ``SELECT`` clause.
...
proc `parameters=`*(query: Query, parameters: Dict) =
    ...
    ##
    ## A parameter named e.g. ``PARAM`` is specified in the query source as ``$PARAM`` (N1QL) or
    ## ``["$PARAM"]`` (JSON). In this example, the `parameters` dictionary
...
proc `parameters=`*(query: Query, json: string) =
    ## Assigns values to the query's parameters, in the form of a JSON string containing an object
    ## whose keys are the parameter names.
