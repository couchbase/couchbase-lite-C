...
def post_coverage_comment(result_file: str, pr_number: str):
    ...
    for key in totals.keys():
        comment_content += f"| {key.capitalize()}"
    comment_content += "|\n"
    ...
    r = octo_client.issues.list_issue_comments(owner="couchbase", repo="couchbase-lite-C", issue_number=pr_number)
    ...
    if comment_id == 0:
        r = octo_client.issues.create_an_issue_comment(owner="couchbase", repo="couchbase-lite-C", issue_number=pr_number, body=comment_content)
        check_response(r, 201, "create_comment")
    else:
        r = octo_client.issues.update_an_issue_comment(owner="couchbase", repo="couchbase-lite-C", 
